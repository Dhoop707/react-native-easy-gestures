...
export default class Gestures extends Component {
  ...
  onDrag(event, gestureState) {
    ...
    if(initialStyles.hasOwnProperty('left')) {
      const left = (isObject ? draggable.x : draggable)
      ? initialStyles.left + gestureState.dx
      : initialStyles.left;
      this.dragStyles['left'] = left;
    }

    if (initialStyles.hasOwnProperty('right')) {
      const right = (isObject ? draggable.x : draggable)
      ? initialStyles.right - gestureState.dx
      : initialStyles.right;
      this.dragStyles['right'] = right;
    }

    if (initialStyles.hasOwnProperty('top')) {
      const top = (isObject ? draggable.y : draggable)
      ? initialStyles.top + gestureState.dy
      : initialStyles.top;
      this.dragStyles['top'] = top;
    }
  
    if (initialStyles.hasOwnProperty('bottom')) {
      const bottom = (isObject ? draggable.y : draggable)
      ? initialStyles.bottom - gestureState.dy
      : initialStyles.bottom;
      this.dragStyles['bottom'] = bottom;
    }
  }
  ...
  reset = (callback) => {
    const { left, top, bottom, right, transform } = this.prevStyles;

    this.dragStyles = { left, top, bottom, right };
    this.pinchStyles = { transform };
